...
class DefaultController extends Controller
{
	...
	public function init(){
        ...
        $this->gridViewClass = $this->module->bootstrap ? 'bootstrap.widgets.BsGridView' : 'zii.widgets.grid.CGridView';
        $this->CButtonColumnClass = $this->module->bootstrap ? 'BsButtonColumn' : 'CButtonColumn';
        parent::init();
    }
    ...
    private function createZipBackup(){
        ...
        if ($zip->open($this->file_name,ZIPARCHIVE::CREATE) === TRUE)
        {
                ...
                $zip->close();
                if($this->module->database)
                    $this->actionDelete($this->back_bd_temp_file,false);
        }
    }

    /**
     * Create Backup (DB and zip from folders seleteds)
     */
    public function createBackup()
    {
        ...
        try{
            ...
            $this->actionDelete($this->back_temp_file,false);
            if($this->module->database)
                $this->actionDelete($this->back_bd_temp_file,false);
            Yii::app()->user->setFlash('error',JBackupTranslator::t('backup','Failed to create the backup:',array('{backup}'=>$this->back_temp_file)));
     ...
     * @param boolean $action is action redirect index 
     * @throws CHttpException If the file does not exist
     */
	public function actionDelete($file = null,$action = true)
	{
		if ( isset($file)){
	...
	public function actionDownload($file = null)
	{
		...
		if ( isset($file))
		{
			...
			if ( file_exists($ziplFile))
			{
				...
				$request->sendFile(basename($file),file_get_contents($ziplFile),'application/octet-stream');
			}
		}
	...
	private function execZipFile($zipFile)
	{
        ...
        if ($res === TRUE){
            ...
            $this->actionDelete(basename($this->module->directoryRestoreBackup.'/database.sql'),false);
        }
	}
		...
		if(isset($file)){
            try{
                ...
                /*Yii::app()->user->setFlash('error',$e->getMessage());*/
                 JBackupTranslator::t('backup','Unable to restore backup correctly:',array('{backup}'=>basename($file)));
            }
        }
		$this->redirect(array('index'));
	}
