	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete','export','import','editable','toggle','assign'),
				'roles'=>array('admin'),
				//'roles'=>array('admin'),
			),
		...
		if(isset($_POST['Usuario']))
		{
			...
			try{
				...
				if($model->save()){
					...
					/*
					$model2 = Usuario::model()->findByPk($model->id);						
					if(!empty($uploadFile)) {
						$extUploadFile = substr($uploadFile, strrpos($uploadFile, '.')+1);
						if(!empty($uploadFile)) {
							if($uploadFile->saveAs(Yii::app()->basePath.DIRECTORY_SEPARATOR.'files'.DIRECTORY_SEPARATOR.'usuario'.DIRECTORY_SEPARATOR.$model2->id.DIRECTORY_SEPARATOR.$model2->id.'.'.$extUploadFile)){
			...
			if (!empty($_FILES)) {
				...
				if (in_array(@$fileParts['extension'],$fileTypes)) {
					...
					while(!empty($sheetData[$baseRow]['A'])){
						...
						$usuario=  $sheetData[$baseRow]['B'];
						$password=  $sheetData[$baseRow]['C'];
						$role=  $sheetData[$baseRow]['D'];

						$model2=new Usuario;
						//$model2->id=  $id;
						$model2->usuario=  $usuario;
